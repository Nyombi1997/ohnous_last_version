(function ($) {
    ...
    form.addEventListener('submit',function(e){e.preventDefault(); if(!iti.isValidNumber()){Swal.fire({icon:'error',title:'Numéro Mobile Money invalide.'});return;} hidden.value=iti.getNumber(); var button=form.querySelector('button[type="submit"]'), old=button.innerHTML; button.disabled=true; button.innerHTML='<i class="fa-solid fa-circle-notch rotate"></i>';
        $.post('/payout-demarrer',$(form).serialize(),function(data){if(data.result==='ok'){Swal.fire({icon:'success',title:'PayOut soumis',text:data.msg||''}).then(function(){window.location.href='/admin-payouts';});}else{Swal.fire({icon:'error',title:data.msg||'PayOut impossible.'});}},'json').fail(function(xhr){Swal.fire({icon:'error',title:(xhr.responseJSON||{}).msg||'PayOut impossible.'});}).always(function(){button.disabled=false;button.innerHTML=old;});
    });
})(jQuery);
