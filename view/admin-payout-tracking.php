...
$final = in_array($status, ['success','successful','paid','completed','failed','error','expired','cancelled','canceled','rejected','refused','declined','manual_review','send_rejected'], true);
$successful = in_array($status, ['success','successful','paid','completed'], true);
...
<main class="admin-shell admin-detail-page">
    ...
    <section class="admin-detail-card payout-tracking-card" id="payout_tracking" data-reference="<?= htmlspecialchars($reference, ENT_QUOTES, 'UTF-8') ?>" data-final="<?= $final ? '1' : '0' ?>">
        <div class="payment-tracking-status">
            <div class="payment-tracking-status__icon <?= $final ? '' : 'is-pending' ?>" id="payout_tracking_icon"><?php if(!$final): ?><span class="payment-tracking-spinner"></span><?php else: ?><i class="fa-solid <?= $successful ? 'fa-circle-check' : 'fa-circle-exclamation' ?>"></i><?php endif ?></div>
            <span class="admin-hero__eyebrow">Suivi en temps réel</span>
            <h1 id="payout_tracking_title"><?= $review ? 'PayOut à rapprocher' : ($successful ? 'PayOut réussi' : ($final ? 'PayOut non abouti' : 'PayOut en cours')) ?></h1>
            <p id="payout_tracking_message"><?= htmlspecialchars($payout['status_description'] ?: 'Le prestataire traite votre demande.', ENT_QUOTES, 'UTF-8') ?></p>
        </div>
        <dl class="admin-detail-list">
            ...
            <div><dt>Référence prestataire</dt><dd id="payout_tracking_freshpay"><?= htmlspecialchars($payout['moko_payout_id'] ?? $payout['freshpay_reference'] ?:  '—', ENT_QUOTES, 'UTF-8') ?></dd></div>
